      ...
      <table class="min-w-full divide-y divide-edge">
        <thead class="bg-surface-alt">
          <tr>
            <th
              ...
              :key="col.key"
              class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-fg-muted uppercase tracking-wider"
              :class="{ 'cursor-pointer hover:text-fg-secondary select-none': col.sortable }"
        ...
        <tbody class="divide-y divide-edge">
          ...
          <tr v-else-if="items.length === 0" class="text-center">
            <td :colspan="columns.length" class="px-3 sm:px-4 py-8 text-fg-subtle text-sm">No data</td>
          </tr>
          <tr
            v-else
            v-for="item in items"
            :key="(item as any).id"
            class="row-hover hover:bg-surface-alt cursor-pointer"
            @click="$emit('row-click', item)"
          >
            <td v-for="col in columns" :key="col.key" class="px-3 sm:px-4 py-3 text-sm text-fg-secondary whitespace-nowrap">
