  ...
  <div>
    <div class="flex items-center justify-between mb-6 gap-4">
      ...
      <div class="flex gap-2">
        <FillExampleSelect v-if="!isEdit" :presets="trackerPresets" @select="fillExample" />
        ...
        <VButton variant="secondary" to="/task-trackers">Cancel</VButton>
      </div>
    </div>
    ...
    <form v-else @submit.prevent="handleSave" class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl mx-auto">
      ...
      <FormField label="Fetch Prompt" :error="fieldError('fetchPrompt')">
        <VTextarea v-model="entity.fetchPrompt" :rows="6" />
      </FormField>
...
Если текста задачи недостаточно, то иди в родительскую задачу.`,
  },
}
