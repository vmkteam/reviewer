...
Синтаксис mermaid — диаграмма с ошибкой парсинга не отрендерится, поэтому строго соблюдай:
- ID участника не должен совпадать с ключевым словом mermaid (loop, end, opt, alt, else, par, and, note, box, break, critical, rect, activate, deactivate) ни в каком регистре: "participant Loop as ..." ломает парсер. Используй короткий нейтральный ID с алиасом: "participant L as Agent Loop" — в алиасе после "as" эти слова допустимы.
- Не используй точку с запятой в тексте сообщений (после ":") — ";" обрывает выражение; разделяй действия запятой.
- Ветка else существует только внутри alt; в opt её нет.
