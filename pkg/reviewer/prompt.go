...
- review.externalId, review.title, review.sourceBranch, review.targetBranch в оригинальном json определены как переменные "EXTERNAL_ID", "TITLE", "SOURCE_BRANCH", "TARGET_BRANCH",
  ...
  Если значения не были заменены на CI и всё ещё содержат шаблонные имена переменных вместо реальных данных — замени их самостоятельно из контекста git:
  - externalId: пустая строка (при локальном запуске нет внешнего ID)
  - title: краткое описание MR из коммитов
  - sourceBranch: текущая git ветка
  - targetBranch: целевая ветка (master/main)
  ВАЖНО: если externalId, title, sourceBranch, targetBranch уже содержат реальные данные (число, текст ветки) — используй их как есть, НЕ перезаписывай.
- review.effortMinutes — оценка времени в минутах для ручного review этого MR опытным разработчиком. Учитывай размер diff, сложность изменений, количество issues, контекст бизнес-логики. Шкала: 5 = тривиальный, 15 = средний, 30 = большой, 60+ = requires architecture discussion.
- issues[].suggestedFix — для каждого issue добавь конкретный код исправления. Формат: markdown code block с указанием языка. Содержит минимальный snippet, решающий проблему. Если fix невозможно предложить без большего контекста — пустая строка.
- review.aiSlopScore — число от 0.0 до 1.0, оценка вероятности того, что MR сгенерирован AI без должного human review (vibe coding / AI slop). Признаки: избыточные шаблонные docstrings, чрезмерная обработка ошибок для невозможных сценариев, однотипные повторяющиеся паттерны, лишние абстракции "на будущее", несоответствие стилю проекта, огромный diff с поверхностными изменениями. Шкала: 0.0 = точно человек, 0.3 = подозрительно, 0.7 = высокая вероятность AI, 1.0 = AI-slop. Для обычного human-written MR ставь 0.0-0.1.
`
